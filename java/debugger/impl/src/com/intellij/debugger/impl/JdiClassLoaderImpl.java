...
public class JdiClassLoaderImpl implements JdiClassLoader {
  @Override
  @Nullable
  public ClassType findOrLoadClass(Class<?> cls, EvaluationContextImpl evaluationContext,
  ...
  private static boolean defineClass(String name,
    ...
    try (InputStream stream = cls.getResourceAsStream('/' + name.replace('.', '/') + ".class")) {
      ...
      ClassLoadingUtils.defineClass(name, stream.readAllBytes(), evaluationContext, evaluationContext.getDebugProcess(), classLoader);
      return true;
    }
