...
/*
 * This class is used to perform smart step target filtering in both Intellij IDEA and Android Studio
 */
object KotlinSmartSteppingUtils {
    suspend fun shouldBeFilteredOut(
        target: KotlinMethodSmartStepTarget,
        debugProcess: DebugProcessImpl,
        bytecodeSignature: BytecodeSignature,
        currentCount: Int
    ): Boolean {
        val methodInfo = target.methodInfo
        val updatedBytecodeSignature = bytecodeSignature
        ...
        return matches(target, debugProcess, updatedBytecodeSignature, currentCount)
    }
}

data class BytecodeSignature(val owner: String, val name: String, val signature: String)
...
private suspend fun matches(
    ...
    val (owner, name, signature) = bytecodeSignature
    if (target.ordinal != currentCount) {
        return false
    }
    ...
    val declaration = target.getDeclaration() ?: return methodInfo.isInvoke
